...
from lib.data.features import FeatureExtractor
from lib.data.features.indicators import add_indicators


def load_from_csv(self):
    raw_df= pd.read_csv("")
    fe = FeatureExtractor(raw_df)
    df1 = fe.add_bar_features()
    df2 = fe.add_mv_avg_features()
    df3 = fe.add_adj_features()
    df4 = fe.add_ta_features()
    df5 = add_indicators(df4)
    ...
    self.closingPrices = self.df['close'].value
